...
public class QuestionController {
    ...
    public Result<Page<Question>> getQuestionList(
        ...
        //方案二：自定义多表查询语句
//        questionService.getQuestionList(questionPage, questionQueryVo);
        //方案三：使用MyBatis Plus的方法全部查询，然后java中进行处理
        questionService.getQuestionListByStream(questionPage, questionQueryVo);
        log.info("查询第{}页题目列表，返回结果为{}", page,questionPage.getRecords());
    ...
    public Result<Question> getQuestionById(
        ...
        log.info("查询题目ID为{}的题目详情成功：{}", id, question);
    ...
    public Result<Question> updateQuestion(
        ...
        log.info("更新题目ID为{}的题目成功：{}", id, question);
        return Result.success(question);
    }
    ...
    public Result<List<Question>> getPopularQuestions(
         ...
         log.info("获取热门题目成功，返回{}条数据", popularQuestions.size());
        // 异常处理：记录日志并返回友好的错误信息
